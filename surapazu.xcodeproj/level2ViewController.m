...
//  surapazu
//
//  Created by 津川 on 2015/11/18.
//  Copyright (c) 2015年 津川. All rights reserved.
//
...
//②⑤
-(IBAction)performChooeImageButton:(id)sender
{
    if([self isPlaying])
        return;
...
-(void)movePieceFromPoint:(CGPoint)point withAnimation:(BOOL)animation
{
    ...
    if (self.pointOfBlank.x == point.x)
        step = self.pointOfBlank.y > point.y ? kNumberOfPieces : -kNumberOfPieces;
    else
...
-(void)totchBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    if(![self isPlaying])
        return;
    //タッチを示すオブジェクトを取得
